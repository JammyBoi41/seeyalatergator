...
const createListing = async(req, res) => {
    try {
        const {title, description, price, userID, userEmail, thumbnail, condition, category} = req.body;
        ...
        const listingResult = await(Listing.create({title, description, price, condition, category, userID, userEmail, thumbnail}));
    } catch(err) {
        res.json({err: err})
    }
}
